...
\begin{abstract}	
     ...
     \par Full-waveform (FW) LiDAR is particularly useful in forestry because it can collect detailed information between the tree canopy, branches and the ground.  However, in comparison to discrete LiDAR, there are very few researchers using it due to the complexity. DASOS, an open source program, implements and proves the novel algorithms developed in this thesis to ease the usage of waveform data. DASOS uses voxelisation for interpreting the data and this approach is fundamentally different from state-of-art tools. There are three key features of DASOS, reflecting key contributions of this thesis:
     \par Visualisations of forests improves field work planning but requires fast algorithms and efficient storage.  Polygonal meshes are generated using DASOS, by extracting an iso-surface from the voxelised data. The concepts of implicit objects and the Marching Cubes were introduced to LIDAR Remote Sensing of forestry. Additionally, six data structures are tested for optimising iso-surface extraction. The new structure, `Integral Volumes', is fastest but the best choice depends on the size of the data.
     \par FW LiDAR data were efficiently aligned with hyperspectral imagery using a geo-spatial representation stored within a hashed table with buckets of points. The outputs of DASOS are coloured polygonal meshes, which improves the visual output, and aligned metrics from the LiDAR and imagery. The metrics were used for generating tree coverage maps and it was demonstrated that the increased amount of information through using both sources of data improves classification versus the results possible with just a single source.
     \par Extraction of feature vectors that characterise objects, such as trees, in 3D. This is used for detecting dead standing eucalyptus trees in a native Australian forest for managing biodiversity without tree delineation.  Random forest, a weighted-distance knn algorithm and a seed growth algorithm were used to predict positions of dead trees.  While noise in the field data used for validation prevented expected improvements in the results from the increased amount of training samples, it nevertheless demonstrated that forest health assessment without tree delineation is possible. Cleaner training samples that are adjustable to tree heights would have produced finer results.  
     \par These novel contributions demonstrate the value in extending advanced algorithms and custom data structures from the computing research field to applied research in cutting-edge remote sensing, with potential real world impacts in forestry.
