...
\section{Classification Challenges {\color{red} *** NEILL : everything from here is new ***}}\label{sec:ClassficationChallenges}
...
\subsection{Random Forest}\label{sec:RandomForest}
	...
	\par Random Forest is able to identify the importance of predicting variables. At first, it generates multiple regression trees by randomly sampling the data at its nodes and choosing the best predicting variables for each sampled data. The variable importance is then defined according to influence it has to the classification once this variable is modified and the rest remain unchanged \cite{Liaw2002}. In this project, the R package is used for finding the most relevant feature of the 3D priors (Section \ref{sec:3DpriorsGeneration} in identifying dead trees. 
		...
		\begin{figure} [h!]			
			...
			\end{subfigure}
			\caption{Application of K-NN using the most significant features identified by Random Forest and filtering.}  
...
\subsection{K-Nearest Neighbour, Filtering and Smoothing}\label{sec:knn}

\par {\color{red} Please skip kNN chapter for now, since I got stuck on writing the equations }
\par Once the ten most significant variables are identified using the Random Forest, the $k-$nearest neighbour algorithm is applied to generate a probabilistic field. Within the algorithm, each selected feature ($F={f_1, f_2, ... , f_3 }$) is associated with a weight according to its importance ($W = {w_1, w_2, ..., w_{10}}$). Additionally the number $k$ is set to 7, because the training datasets are relatively small and they may contain wrong data due to the low accuracy of the field data (Section \ref{sec:ClassficationChallenges}). 


There are also positive training datasets (dead) and negative (alive). 
...
\end{eqnarray}
\par SET UP
\par From DASOS we export training feature vectors of dead and alive trees. Let’s assume that  $T$ is the training datasets and contains $n$ feature vectors. 
\begin{eqnarray}
T  : (x_n, f(x_n)), n=1..N. 
\end{eqnarray}

where $f(x_n) \in \{\textrm{0, 1} \}$, 0 for alive trees and 1 for dead trees

To reduce bias, the number of dead and alive dead trees used in the same for each test dataset. 

For example the dataset $T$ could be like that: $ T: (\boldsymbol{t_1}, alive), (\boldsymbol{t_2}, dead), (\boldsymbol{t_3}, dead), (\boldsymbol{t_4}, alive) . ...... (\boldsymbol{t_n}, dead)$


\par Each instance if $\boldsymbol{t}$ is a feature vector in $T$ then it contains the $10$ most important features ($\boldsymbol{t}={t_1, t_2, … , t_10}$) as they were identified from the Random Forest algorithm. Additionally, every feature has a weight associated with it ($\boldsymbol{w}={w_1,w_2 ... ,w_10} $)


A instance $x_q$ is a vector with on D dimensional space, it features are ${x_1, ….., x_10}$. This is what we want to classify. 


CALCULATING DISTANCES BETWEEN TWO FEATURE VECTORS 
Feature-weighting function dw – xq is the query instance, x is a member of the sample set T, w denotes the weight associated with feature k.


\begin{equation} 
d(\boldsymbol{t},\boldsymbol{x}) = \sqrt{\sum_{k=1}^{10}{ \Big(w_k \times (t_k-x_k)^2 \Big)}}
\end{equation}




SELECTING THE FIVE CLOSEST NEIGHBOURS
Here n* denotes the index of the training sample belonging to set T that is closest to the query instance xq. 
\begin{equation} 
q = \operatornamewithlimits{argmin}_{\boldsymbol{t} \in T} d(\boldsymbol{t},\boldsymbol{x})
\end{equation}
 We have found the 7 closest instances. $V = \{\boldsymbol{v_1}, \boldsymbol{v_2}, \boldsymbol{v_3}, \boldsymbol{v_4}, \boldsymbol{v_5}, \boldsymbol{v_6}, \boldsymbol{v_7}\} $

ASIGNMENT OF WEIGHT: the weight ($u$) is calculated according to the distance from each one of the close vectors 

\begin{equation} 
u_k = \frac{1}{d(\boldsymbol{t_k}, \boldsymbol{x})}
\end{equation}



CREATE PROBABILISTIC FIELD 
The target function can be either discrete valued or real valued. Depending on that the result of the classification will either be from the set {dead, alive} or it will be a real number between 0 and 1.
If discrete values target function then take vote among its k nearest neighbours, else if real valued target function then take the mean of the f values of the k nearest neighbours.

The probability of a dead tree is given by the following equation: 
...
\end{equation}
 ...
 Salt and Pepper noise  appears on the results (Figure \ref{fig:c1_knn}) because of empty columns existing within the volume. This is removed (Figure \ref{fig:c2_SaltPepper}) using a median filter which assigns to every empty pixel the median value of its non-empty neighbouring pixels. By the end a smoothing filter is applying for further noise reduction (Figure \ref{fig:c3_Smoothed}). 
