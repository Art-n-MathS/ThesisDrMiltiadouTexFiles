...
\begin{table}[!h]
	\centering
	\begin{tabular}{|p{0.85cm}|p{4cm}|p{8.5cm}|}
		\toprule
		\multicolumn{3}{|c|}{\textbf{Explanation of some features of DASOS's 3D priors that proved to be useful}}\\ 
		\multicolumn{3}{|c|}{\textbf{for building the classifier}} \\
		\midrule
...
\par Once the ten most significant features are identified using the Random Forest, the weighted k-nearest neighbour algorithm is applied. Since the number of training data is relatively small and they may contain wrong data due to the low accuracy of the field data (Section \ref{sec:ClassficationChallenges}, the number of nearest neighbours kept is $5$.  
...
\end{eqnarray}
 ...
 Salt and Pepper noise  appears on the results (Figure \ref{fig:c1_knn}) because of empty columns existing within the volume. This is removed (Figure \ref{fig:c2_SaltPepper}) using a median filter which assigns to an empty pixel the median value of its non-empty neighbouring pixels. By the end a smoothing filter is applying for further noise reduction (Figure \ref{fig:c3_Smoothed}). 

 
 \subsection{Ground Mask}\label{sec:groundMask}
 ...
 \subsection{Threshold, Segmentation and Position Assignment}\label{sec:Segmentation}
